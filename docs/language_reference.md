...
request         = input_image_field
                  [ input_format_field ]
                  [ output_format_field ]
                  [ operations_field ] ;

input_image_field  = "input_image"  "=" bytes ;
input_format_field = "input_format" "=" format_string ;
output_format_field= "output_format""=" format_string ;
operations_field   = "operations"   "=" "[" { operation } "]" ;

(* ── Format strings ────────────────────────────────────────────── *)

format_string   = "JPEG" | "PNG" | "GIF" | "BMP" | "WEBP" | "TIFF" ;
                  (* case-insensitive; auto-detected from image bytes if omitted *)

(* ── Operation (exactly one variant per element) ───────────────── *)
...
rotate_degrees  = "rotate_degrees" "{" "angle"      "=" float   "}" ;
resize          = "resize"         "{" "percent"     "=" integer "}" ;
thumbnail       = "thumbnail"      "{" "max_width"   "=" integer ","
                                       "max_height"  "=" integer "}" ;
...
bytes   = (* raw binary image bytes, base-64 encoded in JSON *) ;
```
